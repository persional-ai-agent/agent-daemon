...
- `agentd setup`：统一写入最小模型/provider 配置，可选连带写入一个 gateway 平台配置，作为轻量 setup wizard 替代。
...
- 配置与 CLI 管理面：已具备最小 `config list|get|set`、`model show|providers|set`、`tools list|show|schemas|enable|disable`、`doctor`、`setup`、`gateway status|platforms|enable|disable|setup`；后续补齐交互式 setup wizard 等命令入口。
...
| CLI/TUI | `cli.py`、`hermes_cli/*`、`ui-tui/` | `internal/cli/chat.go`、`cmd/agentd`、`internal/config/manage.go` | 部分对齐 | 已补最小 config/model/tools 查看与启停/doctor/`setup`/gateway 开关与 `gateway setup`；后续补交互式 setup wizard，再评估 TUI |
