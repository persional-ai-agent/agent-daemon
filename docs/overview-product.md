...
| CLI/TUI | 部分对齐 | 已有交互式 chat、serve、tools list/show/schemas/enable/disable、config、model、doctor、`setup`/`setup wizard`、`bootstrap`、`version`、最小 `update`、gateway 与最小 `gateway setup/run/start/stop/restart/install/uninstall`；未实现 Hermes 全屏 TUI、安装器级 update 与更完整命令体系 |
