...
| `TODO-005` | `partial` | 已补 `matrix` + `feishu` + `dingtalk` + `wecom` + `mattermost` 网关适配器（inbound/outbound 最小闭环）；`sms/bluebubbles` 仍待实现。 | `internal/gateway/platforms/matrix.go`、`internal/gateway/platforms/feishu.go`、`internal/gateway/platforms/dingtalk.go`、`internal/gateway/platforms/wecom.go`、`internal/gateway/platforms/mattermost.go`、`cmd/agentd/main.go` |
