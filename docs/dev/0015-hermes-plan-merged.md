...
| `TODO-005` | `partial` | 已补 `matrix` + `feishu` 网关适配器（inbound/outbound 最小闭环）；`dingtalk/wecom/mattermost/sms/bluebubbles` 仍待实现。 | `internal/gateway/platforms/matrix.go`、`internal/gateway/platforms/feishu.go`、`cmd/agentd/main.go` |
