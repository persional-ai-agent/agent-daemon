...
| `TODO-004` | `partial` | 已补 `webhook` + `signal` + `email` 网关适配器（inbound/outbound 最小闭环）；`home_assistant` 仍待实现。 | `internal/gateway/platforms/webhook.go`、`internal/gateway/platforms/signal.go`、`internal/gateway/platforms/email.go`、`cmd/agentd/main.go` |
