...
### 来源：`0290-gateway-grant-revoke-unified-context-parser-bundle.md`

# 290-summary-gateway-grant-revoke-unified-context-parser-bundle

本轮继续推进 `TODO-002`，将 grant/revoke 参数解析切到统一命令上下文（`head + args`），去掉审批管理路径中的二次字符串拆分。

## 变更

- `internal/gateway/runner.go`
  - `grantApproval` / `revokeApproval` 改为接收 `gatewayCommand`（统一解析结果），不再接收原始字符串。
  - `parseApprovalManageCommand` 改签名为：
    - `parseApprovalManageCommand(head string, args []string)`
  - 解析逻辑统一基于 canonical head 与 args，避免在函数内重复 `strings.Fields` 分解。
  - pattern 与 ttl 错误提示改为复用 catalog usage 导出函数。

- `internal/gateway/runner_command_test.go`
  - 新增 `TestParseApprovalManageCommand`，覆盖：
    - grant pattern + ttl
    - revoke pattern
    - grant 错误 ttl
    - pattern 参数缺失

- `internal/gateway/commands.go` / `commands_test.go`
  - 新增 `GatewayGrantPatternOrRevokePatternUsage()` 并补回归断言。
...
### 来源：`0291-gateway-command-auth-policy-catalog-bundle.md`

# 291-summary-gateway-command-auth-policy-catalog-bundle

本轮继续推进 `TODO-002`，将 Gateway 命令鉴权规则从 runner 分支硬编码迁移到 command catalog 策略函数，统一权限判定入口。

## 变更

- `internal/gateway/commands.go`
  - 新增鉴权策略导出：
    - `GatewayCommandsRequiringAuthorization() []string`
    - `GatewayCommandRequiresAuthorization(name string) bool`
  - 鉴权命令集合集中维护：
    - `cancel/queue/status/approve/deny/approvals/pending/grant/revoke`

- `internal/gateway/runner.go`
  - slash 命令分发前增加统一鉴权前置判断：
    - 使用 `GatewayCommandRequiresAuthorization(parsed.head)`
  - 删除各 case 内重复的 `_Access denied._` 分支（行为保持一致，逻辑去重）。

- `internal/gateway/commands_test.go`
  - 新增 `TestGatewayCommandRequiresAuthorization`：
    - 校验鉴权命令集合完整性
    - 校验 `/pair` 不需要鉴权、`/approve` 需要鉴权等边界
...
### 来源：`0292-gateway-platform-description-from-catalog-bundle.md`

# 292-summary-gateway-platform-description-from-catalog-bundle

本轮继续推进 `TODO-002`，将平台注册命令描述文案统一改为由 gateway command catalog 提供，进一步减少平台层与核心层的文案双份维护。

## 变更

- `internal/gateway/commands.go`
  - `gatewayCommandSpec` 增加 `Description` 字段，并为全部内建命令补齐描述。
  - 新增导出函数：
    - `GatewayCommandOrder() []string`
    - `GatewayCommandDescriptions() map[string]string`
  - 供平台层按统一顺序和描述生成注册命令。

- `internal/gateway/platforms/command_specs.go`
  - `GatewayPlatformCommandSpecs()` 改为从 `gateway.GatewayCommandOrder()` + `gateway.GatewayCommandDescriptions()` 自动生成基础 spec。
  - 平台层仅保留 Discord 特有参数定义（`code/pattern/ttl/id`），命令名与描述不再重复硬编码。

- `internal/gateway/commands_test.go`
  - 新增 `TestGatewayCommandOrderAndDescriptions`，校验命令顺序唯一且所有命令描述非空。
