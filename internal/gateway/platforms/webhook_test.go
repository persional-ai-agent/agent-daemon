...
func TestWebhookAdapterSend(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotSecret = r.Header.Get("X-Agent-Webhook-Secret")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"message_id":"m-1"}`))
	}))
	defer srv.Close()

	adapter, err := NewWebhookAdapter(srv.URL, "s3")
	if err != nil {
...
func TestWebhookAdapterHandleWebhook(t *testing.T) {
	adapter, err := NewWebhookAdapter("http://example.test/out", "secret")
	if err != nil {
		t.Fatalf("new adapter: %v", err)
	}
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/gateway/webhook/inbound", strings.NewReader(body))
	req.Header.Set("X-Agent-Webhook-Secret", "secret")
...
func TestWebhookAdapterHandleWebhookForbidden(t *testing.T) {
	adapter, err := NewWebhookAdapter("http://example.test/out", "secret")
	if err != nil {
		t.Fatalf("new adapter: %v", err)
	}
	req := httptest.NewRequest(http.MethodPost, "/v1/gateway/webhook/inbound", strings.NewReader(`{"text":"hi","chat_id":"c1"}`))
	rec := httptest.NewRecorder()
	adapter.HandleWebhook(rec, req)
