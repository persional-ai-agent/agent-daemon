...
func NewSMSAdapter(outboundURL, inboundSecret string) (*SMSAdapter, error) {
	outboundURL = strings.TrimSpace(outboundURL)
	if outboundURL == "" {
		return nil, errors.New("sms outbound url is required")
	}
	return &SMSAdapter{
...
func (w *SMSAdapter) Name() string { return "sms" }

func (w *SMSAdapter) Connect(_ context.Context) error { return nil }

func (w *SMSAdapter) Disconnect(_ context.Context) error { return nil }

func (w *SMSAdapter) Send(ctx context.Context, chatID, content, replyTo string) (gateway.SendResult, error) {
	...
	if w.inboundSecret != "" {
		req.Header.Set("X-Agent-SMS-Secret", w.inboundSecret)
	}
	client := w.httpClient
	if client == nil {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return gateway.SendResult{Success: false, Error: fmt.Sprintf("sms send failed: %s", msg)}, nil
	}
	messageID := ""
	var out struct {
...
func (w *SMSAdapter) EditMessage(_ context.Context, _, _, _ string) error {
	return errors.New("sms does not support edit message")
}

func (w *SMSAdapter) SendTyping(_ context.Context, _ string) error { return nil }

func (w *SMSAdapter) OnMessage(_ context.Context, handler gateway.MessageHandler) {
	w.handler = handler
}

func (w *SMSAdapter) HandleWebhook(resp http.ResponseWriter, req *http.Request) {
	if req.Method != http.MethodPost {
		http.Error(resp, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	if w.inboundSecret != "" {
		if strings.TrimSpace(req.Header.Get("X-Agent-SMS-Secret")) != w.inboundSecret {
