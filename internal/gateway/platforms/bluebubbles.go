...
func NewBlueBubblesAdapter(outboundURL, inboundSecret string) (*BlueBubblesAdapter, error) {
	outboundURL = strings.TrimSpace(outboundURL)
	if outboundURL == "" {
		return nil, errors.New("bluebubbles outbound url is required")
	}
...
func (w *BlueBubblesAdapter) Name() string { return "bluebubbles" }

func (w *BlueBubblesAdapter) Connect(_ context.Context) error { return nil }

func (w *BlueBubblesAdapter) Disconnect(_ context.Context) error { return nil }

func (w *BlueBubblesAdapter) Send(ctx context.Context, chatID, content, replyTo string) (gateway.SendResult, error) {
	...
	if w.inboundSecret != "" {
		req.Header.Set("X-Agent-BlueBubbles-Secret", w.inboundSecret)
	}
	client := w.httpClient
	if client == nil {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return gateway.SendResult{Success: false, Error: fmt.Sprintf("bluebubbles send failed: %s", msg)}, nil
	}
	messageID := ""
	var out struct {
...
func (w *BlueBubblesAdapter) EditMessage(_ context.Context, _, _, _ string) error {
	return errors.New("bluebubbles does not support edit message")
}

func (w *BlueBubblesAdapter) SendTyping(_ context.Context, _ string) error { return nil }

func (w *BlueBubblesAdapter) OnMessage(_ context.Context, handler gateway.MessageHandler) {
	w.handler = handler
}

func (w *BlueBubblesAdapter) HandleWebhook(resp http.ResponseWriter, req *http.Request) {
	if req.Method != http.MethodPost {
		http.Error(resp, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	if w.inboundSecret != "" {
		if strings.TrimSpace(req.Header.Get("X-Agent-BlueBubbles-Secret")) != w.inboundSecret {
