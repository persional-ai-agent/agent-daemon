...
func TestGatewayFeishuWebhookEndpoint(t *testing.T) {
	srv := &Server{
		Engine: &agent.Engine{
			Client:       fakeModelClient{response: core.Message{Role: "assistant", Content: "ok"}},
			Registry:     tools.NewRegistry(),
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/feishu/webhook", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "feishu"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("feishu") })
...
func TestGatewayWeComWebhookEndpoint(t *testing.T) {
	srv := &Server{
		Engine: &agent.Engine{
			Client:       fakeModelClient{response: core.Message{Role: "assistant", Content: "ok"}},
			Registry:     tools.NewRegistry(),
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/wecom/webhook", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "wecom"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("wecom") })
...
func TestGatewayMattermostWebhookEndpoint(t *testing.T) {
	srv := &Server{
		Engine: &agent.Engine{
			Client:       fakeModelClient{response: core.Message{Role: "assistant", Content: "ok"}},
			Registry:     tools.NewRegistry(),
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/mattermost/webhook", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "mattermost"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("mattermost") })
...
func TestGatewayEmailWebhookEndpoint(t *testing.T) {
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/email/webhook", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "email"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("email") })
...
func TestGatewayHomeAssistantWebhookEndpoint(t *testing.T) {
	srv := &Server{
		Engine: &agent.Engine{
			Client:       fakeModelClient{response: core.Message{Role: "assistant", Content: "ok"}},
			Registry:     tools.NewRegistry(),
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/homeassistant/webhook", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "homeassistant"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("homeassistant") })
...
func TestGatewayWebhookInboundEndpoint(t *testing.T) {
	...
	t.Run("adapter_unavailable", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/gateway/webhook/inbound", bytes.NewBufferString(`{}`))
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusServiceUnavailable {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
	...
	t.Run("adapter_available", func(t *testing.T) {
		adapter := &stubWebhookAdapter{name: "webhook"}
		platform.Register(adapter)
		t.Cleanup(func() { platform.Unregister("webhook") })
...
func TestUIEndpoints(t *testing.T) {
	...
	t.Run("session_branch", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/ui/sessions/branch", bytes.NewBufferString(`{"session_id":"s1","new_session_id":"s-branch","last_n":1}`))
		req.Header.Set("Content-Type", "application/json")
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
		}
		if !strings.Contains(rec.Body.String(), `"new_session_id":"s-branch"`) {
			t.Fatalf("unexpected body: %s", rec.Body.String())
		}
	})
	...
	t.Run("session_replay", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/v1/ui/sessions/replay", bytes.NewBufferString(`{"session_id":"s1","offset":0,"limit":2}`))
		req.Header.Set("Content-Type", "application/json")
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
