...
创建带投递目标的任务时，可在 `cronjob` 工具或 Web Cron 页面设置 `delivery_target`，格式为 `telegram:123`、`discord:channel_id`、`slack:channel_id` 或 `yuanbao:group:123`，并用 `deliver_on=always|success|failure` 控制何时投递。周期任务需要继承上次运行上下文时，可设置 `context_mode=chained`；默认 `isolated` 保持每次运行独立会话。
