...
本项目当前对齐的是 Hermes 的核心 Agent daemon 子集，不是 Hermes Agent 的完整 Go 版复刻。已覆盖核心 Agent Loop、工具调用、会话摘要检索/长期记忆、MCP、Skills、Provider 韧性、HTTP/SSE/WebSocket、Telegram/Discord/Slack/WhatsApp 最小网关、状态化 CLI/TUI slash 命令面，并补齐了插件生态闭环（JSON/YAML manifest 发现/校验、签名与文件校验、默认进程沙箱、tool/provider 插件注册、command 执行、dashboard slot API、本地/marketplace 安装卸载与启停管理）；暂未覆盖 Hermes 的完整高级 TUI、大规模 provider 生态、52 个 toolsets 的完整动态行为、多终端后端高级语义、ACP/Cron 高级能力、完整 Gateway 和研究/训练链路。
...
- SQLite 会话持久化，`session_search` 返回按会话聚合的摘要、关键词、事实与高亮信息
- `MEMORY.md` / `USER.md` 持久记忆，`memory.extract` 可从对话文本抽取稳定事实/偏好并去重
...
- Cron：`cronjob` 工具 + 后台调度器，支持 interval、one-shot、RFC3339 时间戳、5/6 字段 cron 表达式、运行结果投递到已连接 Gateway 目标，以及可选链式上下文（需开启 `AGENT_CRON_ENABLED=true`）
...
go run ./cmd/agentd update bundle manifest -file /tmp/release.tgz -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle plan -file /tmp/release.tgz -dest /tmp/agent-daemon-checkout -json
...
go run ./cmd/agentd update bundle status -file /tmp/release.tgz -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle doctor -dest /tmp/agent-daemon-checkout -json
...
go run ./cmd/agentd update bundle snapshots-doctor -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle snapshots-status -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle snapshots-restore-plan -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle snapshots-restore -dest /tmp/agent-daemon-checkout -json
go run ./cmd/agentd update bundle snapshots-delete -dest /tmp/agent-daemon-checkout -json
...
开启 Cron 调度器（interval/one-shot/cron 表达式，可选投递结果到 Gateway）：
...
创建带投递目标的任务时，可在 `cronjob` 工具或 Web Cron 页面设置 `delivery_target`，格式为 `telegram:123`、`discord:channel_id`、`slack:channel_id`、`whatsapp:8613800138000` 或 `yuanbao:group:123`，并用 `deliver_on=always|success|failure` 控制何时投递。周期任务需要继承上次运行上下文时，可设置 `context_mode=chained`；默认 `isolated` 保持每次运行独立会话。
...
```bash
go run ./cmd/agentd plugins list
go run ./cmd/agentd plugins verify
go run ./cmd/agentd plugins install ./plugins/demo
go run ./cmd/agentd plugins marketplace list -file ./plugins/marketplace.json
go run ./cmd/agentd plugins marketplace install -file ./plugins/marketplace.json demo
go run ./cmd/agentd plugins commands
go run ./cmd/agentd plugins dashboards
go run ./cmd/agentd plugins exec hello --json
go run ./cmd/agentd plugins disable demo
go run ./cmd/agentd plugins enable demo
go run ./cmd/agentd plugins uninstall demo
```
...
go run ./cmd/agentd gateway setup -platform whatsapp -access-token "$AGENT_WHATSAPP_ACCESS_TOKEN" -phone-number-id "$AGENT_WHATSAPP_PHONE_NUMBER_ID"
```
