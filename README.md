...
本项目当前对齐的是 Hermes 的核心 Agent daemon 子集，不是 Hermes Agent 的完整 Go 版复刻。已覆盖核心 Agent Loop、工具调用、会话/记忆、MCP、Skills、Provider 韧性、HTTP/SSE/WebSocket 与 Telegram/Discord/Slack 最小网关，并补齐了最小插件运行时（manifest 发现/校验、tool 插件注册与 CLI 管理）；暂未覆盖 Hermes 的完整 TUI/CLI、provider 插件生态、68 个内置工具、52 个 toolsets、多终端后端、ACP、Cron 高级能力、完整 Gateway 和研究/训练链路。
