...
- `event_type=usage` 时，`event_data.prompt_tokens`、`event_data.completion_tokens`、`event_data.total_tokens`（缺失时自动补齐；异常偏小时自动校正，`event_data.total_tokens_adjusted=true`）、`event_data.prompt_cache_write_tokens`（可选）、`event_data.prompt_cache_read_tokens`（可选）、`event_data.reasoning_tokens`（可选）
